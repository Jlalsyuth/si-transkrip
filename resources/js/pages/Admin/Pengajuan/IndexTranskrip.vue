...
import AppLayout from '@/layouts/AppLayout.vue'; // Asumsi layout yang sesuai
import { Head, Link } from '@inertiajs/vue3';
import { defineProps, ref, computed } from 'vue'; // Tambahkan ref dan computed
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input'; // Untuk search
import { Eye, Search, ListFilter } from 'lucide-vue-next';

// Interface untuk User (Mahasiswa)
interface Mahasiswa {
    id: number;
    nama_lengkap?: string;
    nama?: string;
...
// Interface untuk Status Pengajuan
...
// Interface untuk objek Pengajuan yang diterima
interface Pengajuan {
    id: number;
    mahasiswa: Mahasiswa;
    jenis_pengajuan: string; // Akan 'transkrip_sementara'
    keperluan_transkrip?: string | null;
    bahasa_transkrip?: string | null; // Mungkin ingin ditampilkan juga
    tanggal_pengajuan: string;
    current_status: StatusPengajuan;
}

// Interface untuk data paginasi
...
const breadcrumbs = [
    {
        title: 'Admin Dashboard', // Sesuaikan jika nama route dashboard admin berbeda
        href: route('admin.dashboard'), // Asumsi ada route admin.dashboard
    },
    {
        title: props.pageTitle,
        href: route('admin.pengajuan.transkrip.index'),
        isCurrent: true,
    },
];
...
const filteredPengajuans = computed(() => {
    ...
    return props.pengajuans.data.filter(p =>
        ...
        (p.keperluan_transkrip?.toLowerCase().includes(lowerSearchTerm)) ||
        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p class="text-sm text-muted-foreground">Kelola semua pengajuan transkrip sementara dari mahasiswa.</p>
                </div>
            </div>

            <div class="bg-card text-card-foreground shadow-md rounded-lg border border-border">
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row gap-4 justify-between items-center mb-4">
                        <div class="relative w-full sm:max-w-xs">
                            <Input 
                                type="text"
                                v-model="searchTerm"
                                placeholder="Cari (NIM, Nama, Keperluan, Status)..."
                                class="w-full pl-10"
                            />
                ...
                <div v-if="!pengajuans || pengajuans.data.length === 0" class="text-center py-10 px-6">
                    <p class="text-muted-foreground text-lg">Tidak ada pengajuan transkrip yang ditemukan.</p>
                </div>
                    ...
                    <table v-else class="min-w-full divide-y divide-border">
                        <thead class="bg-muted/50">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Keperluan</th>
